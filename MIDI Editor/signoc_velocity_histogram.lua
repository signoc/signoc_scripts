-- @description Velocity Histogram
...
function read_all_events(vars) 
    ...
    local rv, evts = reaper.MIDI_GetAllEvts(vars.environment.take)
    if not rv then
    ...
    while pos < #evts do
        prevpos = pos
        offset, flags, msg, pos = string.unpack("i4Bs4", evts, pos)
        ppq = ppq + offset
        ...
        if (flags&1) == 1 then
            ...
            ms = sb>>4
            if ms == 9 then
                -- offset(4) + flags(1) + msglen(4) + noteon(1) + pitch(1)
                t_evts[#t_evts + 1] = evts:sub(prevpos,prevpos+10)
                prevpos = prevpos + 4 + 1 + 4 + 1 + 1
                -- velocity 1 byte
                t_evts[#t_evts + 1] = evts:sub(prevpos,prevpos)
                prevpos = prevpos + 1
...
function action_ramp(notes, gui_curve)
...
function update_all_events(take, all_events, notes)
    local e = all_events
    local n = notes
    local t = 0
    local ix = 0
...
function ActionChaos:callback(vars)
    ...
    if self.is_active then
        ...
        update_all_events(vars.environment.take, vars.t_events, vars.t_note_src)
